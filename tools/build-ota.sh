...
# so version.txt, firmware.bin, and merged.bin can never disagree with each other.
#
# ota/ holds everything needed to update or install the firmware, all in one place:
#   version.txt   the version string; `update ota` fetches this to decide if newer
...
#
# Because all three are written from the same `pio run`, they are always in sync within
# a refresh. The `pio test -e native` guard (test/test_ota_payload) then fails if the
# version embedded in the committed binaries drifts from version.txt/version.h — e.g.
# after a version bump where you forgot to re-run this script.
#
...
echo ">> writing ota/version.txt from version.txt"
cp version.txt ota/version.txt
